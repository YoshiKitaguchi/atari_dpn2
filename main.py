...
def select_action(state):
    ...
    if sample > eps_threshold:
        with torch.no_grad():
            return policy_net(state.to('cuda')).argmax().view(1,1)
    else:
...
if __name__ == '__main__':
    ...
    if os.path.exists("dqn_pong_model") and len(sys.argv) == 4 and sys.argv[1] == "train" and sys.argv[3] == "load":
